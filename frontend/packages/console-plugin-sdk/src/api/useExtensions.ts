...
export const useExtensions = <E extends Extension>(
  ...
  const previousResult = useRef<LoadedExtension<E>[]>([]);
  const previousUIDs = useRef<string>('');
  ...
  return useMemo(() => {
    ...
    if (currentUIDs === previousUIDs.current) {
      return previousResult.current;
    }

    // Update refs and return new result
    previousResult.current = sorted;
    previousUIDs.current = currentUIDs;
