    ...
    Object.entries(genericResources).forEach(([key, resource]) => {
      ...
      if (model.version && model.group) {
        ...
        if (!internalModel) {
          ...
          console.warn(
            `Plugin "${uid}": Could not find model (CRD) for group "${model.group}" and kind "${model.kind}" to determine version. Please add a required flag to the extension to suppress this warning. The resource "${key}" will not be loaded and ignored in the topology view for now.`,
          );
          return;
        }
